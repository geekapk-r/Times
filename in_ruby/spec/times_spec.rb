RSpec.describe Times do
  ...
  describe Times::Comment do
    ...
    it 'serializes&deserializes right' do
      ...
      ser = c.ser
      expect(JSON.parse(ser)).to eq({
        'author' => 'boy',
        'text' => 'Oh, my shoudler',
        'created' => Time.at(2333).to_s,
        'likes' => 1
      })
      expect(Times::Comment.de(ser).ser).to eq(c.ser)
      c.text << '\n' << 'Billy replies: Oh f**king c**mming'
      expect(c.ser).to include('Billy')
    end
  end
  ...
  describe Times::Post do
    ...
    it 'serializes&deserializes right' do
      ...
      de = Times::Post.de(ser)
      expect(de.ser).to eq(p.ser)

      p.comment('duangsuse', ':see_no_evil:')
      p.comment('van', 'f**k you')
      expect(Times::Post.de(p.ser).ser).to eq(p.ser)
      expect(Times::Comment.de(JSON.parse(p.ser)['comments'][0]).author).to eq('duangsuse')
      expect(Times::Comment.de(JSON.parse(p.ser)['comments'][0]).text).to eq(':see_no_evil:')
    end
  ...
  it 'serializes well' do
    ...
    ser = Times.ser
    expect(JSON.parse((JSON.parse(ser)[4]))['author']).to eq('billy')
