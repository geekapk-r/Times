RSpec.describe TimesApp do
  ...
  describe 'APIs' do
    ...
    it 'gives comments' do
      ...
      expect(JSON.parse(JSON.parse(last_response.body)[0])['author']).to eq('233')
    end
    ...
    it 'adds comment' do
      ...
      expect(JSON.parse(JSON.parse(last_response.body)[1])['author']).to eq('duangsuse')
    end
    ...
    it 'can post' do
      ...
      expect(last_response).to be_ok
      a = Times.posts.pop
      expect(a.title).to eq(URI.decode_www_form(title))
    ...
    describe 'Realtime API' do
      it 'rejects closed' do
        get '/api/realtime'
        expect(TimesApp.instance_variable_get(:@connections).size).to eq(0)
      end
